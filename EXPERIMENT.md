...
Brier Skill Score 최대화. 1차 목표 **1120대 합법 진입**.
...
## 2026-08-28 overnight autonomous run — two GPU candidates, both FAIL, no submission

노트북(RTX 5060) 로컬 실행. `desktop-5070`은 ssh 타임아웃으로 도달 불가.
판정표면 `--drop-f-pre 2022 --max-train-season 2024 --val-season 2023
--test-season 2024`, 미학습 2024 253,507행. 챔피언 불변, 제출 없음.

**CELL_LEAF_ITERS_10 — FAIL, core −0.381.** 셀 팔의
`leaf_estimation_iterations`를 base 팔과 같은 10으로 맞추는 1변경. 셀 팔은
단독으로 **+2.579**를 얻었으나 코어는 잃었다. 이유를 측정했다:
`corr(base, cell)`이 **0.9647370 → 0.9681003**으로 오르고 셀 sd가 줄었다.
leaf가 잘 수렴할수록 셀 팔이 base 팔에 가까워지고, **블렌드의 가치는 두 팔의
불일치이므로 그것을 소모한다.** 각자 최적 가중에서도 무승부(control w=0.60
889.128 / candidate w=0.65 889.116)라 재가중으로 살릴 수 없다. 선언한 기전
(resolution)은 반증됐다 — resolution이 오히려 떨어졌다.

**CELL_BUDGET_CEILING — FAIL, core −1.056.** 원장 263개 셀 행 중 164개가
`best_iter` 2990–2999에 몰려 있다. `--es 500`은 한 번도 발동한 적이 없고,
`--refit-mult 1.5`가 그 잘린 수를 곱한다. 예산을 8000으로 주자 조기종료가
실제로 발동했고(**best_iter 3813**), 이 팔이 28% 더 많은 트리를 원한다는 것이
현 레시피에서 확증됐다. 그런데 성능은 떨어졌다. **사전등록한 다양성 기전
검정은 통과했다** — `corr(base, cell)`이 **0.9617575로 내려갔고** 셀 sd는
올랐다. 진짜 capacity 변경이었고 다양성도 실제로 늘었는데 팔이 나빠졌다.
기전은 **검증 시즌 과적합**이다: 선택 표면 `val2023 589.88 → 590.70`이
좋아지는 동안 미학습 2024 셀 BSS는 5.56 떨어진다.

**따라서 `--iters 3000` 상한은 셀 팔이 겪는 결함이 아니라 셀 팔이 의존하는
암묵적 정규화다.** D12(체크포인트 *기준* 변경, CLOSED)와 함께 읽으면, 셀 팔의
체크포인트 기계는 기준으로도 예산으로도 미학습 시즌 성능을 추적하지 못하며
현 설정은 상한이 그것을 보정해서 살아 있다. 4000/5000/6000/10000/12000 재시도,
`--es` 변경, `--refit-mult` 변경 모두 금지.

**이번 밤의 이전 가능한 교훈**: **한 팔을 단독으로 개선하는 것과 코어를
개선하는 것은 같지 않다.** 두 팔을 더 일치시키는 개입은 LEAFIT의 서명(단독 +,
코어 −)을 보이고, 다양성을 늘리는 개입도 그 팔이 미학습 시즌에서 나빠지면
소용없다.

**인프라 2건 수정.** `tools/precheck.py`는 첫 인자가 `--file`이 아니면 파일을
열지도 않고 그럴듯한 `[OK]`를 찍은 뒤 exit 0을 냈다 — 모든 GPU 실행의 관문이다.
미지 옵션·경로 형태 인자·무인자를 모두 exit 2로 거부하고, 미지 판정은
트레이너 argparse를 소스에서 읽어 하므로 새 플래그가 오탐되지 않는다
(`tests/test_precheck_dispatch.py`, 9검사). `tools/leafit_gate.py`의 무결성
계약은 `iterations`/`tree_count`/`best_iteration`이 **설정이 아니라 결과**임을
반영해 변경 변수를 각 실행의 lineage `resolved_args`에서 검증한다.

**Provenance.** `docs/CHAMPION_CONSTANT_PROVENANCE_20260828.md` — 챔피언의
상수 7개 중 6개는 학습데이터에서 유도 가능하고 독립 재검증됐다. 하나,
`+0.002515795361`은 **리더보드 점수 3개에 2차식을 적합한 값**이라 `train.csv`
만으로 재유도할 수 없다. 기록만 했고 바꾸지 않았다.
...
p99 미달이다. 챔피언·제출은 불변이다. 상세 수치와 최종 검토 근거는
[docs/BND_AND_RESOLUTION_REPORT_20260816.md](docs/BND_AND_RESOLUTION_REPORT_20260816.md).

사용자 요청에 따라 Codex가 원장·lineage·구현을 다시 읽고 세 CPU 감사를
결정론적으로 재실행했으며 결과가 완전히 재현됐다. 최종 판정은 SETTLED에
append-only로 기록했다. 다음 실행 큐는 비어 있다.

## 2026-08-16 GSK 챔피언 검토 (Claude) — 챔피언 유지, E-LB2 취소

ZIP을 문서가 아니라 실물로 검증했다. 22멤버, base 8팩은 `b1s8_20260813.zip`과
**sha256 동일**, cell 6팩은 `fit_rowid_sha d69792676c50e1cf` / 123피처 공유,
`set(cell)-set(base) == ['skill_hat','skill_hat_vs_std']`. `failmode.py` 부재이고
인라인 등가물도 없다(패키지 전체 `.diff(` 0건, 모든 교차행 집계는 fit-time 빌더
안에 갇혀 `fpipe.transform`에서 도달 불가). 강한 부분집합 독립성은 논증이 아니라
실측으로 **0.000e+00**(절반/투수단독/산개/단일행/역순). **SAFE TO KEEP.**

**해석 정정 — "13%만 재현됐다 / 전이 감쇠"는 철회한다.** `+3.108`·`+19.517`은
판정 표면 숫자다. 제출 표면 추정치는 이미 기록돼 있었고(`GENERAL_SKILL_ADD`
core **+1.104, SE 1.086**), 저장된 val2024 배열로 cell 팔만 교체해 재계산하면
**956.136 → 957.478 = +1.342**다. 그리고 **LB 델타 자체의 표본오차가 ±1.06**이다
(보정 후 `rms(p_new−p_old)=0.001323`, 245,789행). 관측된 **+0.391**은 +1.342에서
**0.9 SE**, +1.104에서 0.66 SE 떨어져 있다. 감쇠는 없다 — 올바른 표면을 쓰면
제출 표면 debiased delta가 LB를 1 SE 안에서 맞혔다는 것이 진짜 교훈이다.

**운영상 귀결(구속력 있음)**: 판정 표면 delta를 기대 LB 이득으로 인용하지 않는다.
그리고 **LB는 이 크기의 변경에 대해 +2~+3 미만을 분해하지 못한다** — 제출 1회는
SE 1의 draw 한 번이다.

**E-LB2 취소.** GSK cell을 넣은 val2024 가중 곡선은 `.30~.80`에서
`953.45 … 957.48(.55) … **957.55(.60)** … 955.42`로, 최적점 이득이 **+0.07**이고
`.45~.65` 전 구간이 0.78점 안에 든다. ±1.06 잡음 대비 S/N 0.066이라 두 번의 제출은
잡음에 이차식을 맞추게 된다. `_W_CELL`은 0.55 유지, 계수 이식 금지.

다음 후보·기대값·게이트: [docs/NEXT_CANDIDATES_20260816.md](docs/NEXT_CANDIDATES_20260816.md)
