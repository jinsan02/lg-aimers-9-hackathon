...
`RUNNING_CODEX_SOLO` — Claude 토큰 제약으로 임시 Codex 단독 운영 중. 아래의 과거
`READY_FOR_CLAUDE` 블록은 역사 기록이며 현재 배턴이 아니다.

- 노트북: 문서·검문 도구·결과 판정 전용.
- A100: `TA1_anchor` (`--feat-anchor`) 6시드 실행 중. 당해 시즌을 제외한 순수 과거
  n0/S0를 노출하며, missing 플래그는 섞지 않았다.
- 4070: 기존 예측 재분석 완료, 현재 유휴.
