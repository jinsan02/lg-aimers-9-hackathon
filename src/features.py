...
def add_abs_regime(df):
    """역사적으로 'ABS'라 부른 타깃/데이터 체제 proxy.

    근거(tools/kbo_calendar.py 실측):
      F리그 2022 0.7087 → 2023 0.4729 (-0.236)   ← 데이터 체제 단절
      R리그 2023 0.5031 → 2024 0.4897 (-0.013)   ← 1군 ABS 도입
    타깃이 존 위치 기반이므로 이건 선수 실력이 아니라 **측정 체계의 변화**다.

    ⚠ 2026-08-09 공식 연혁 재확인: 퓨처스리그 ABS는 2020년부터 운영됐다.
    F 2022→2023의 reverse .0497→.2988 급변은 ABS 최초 도입이 아니다. 아래
    변수명과 경계는 호환성 때문에 유지할 뿐 정책 사실로 해석하지 않는다.

    핵심: 이 체제 구분은 (game_type=='F' AND season>=2023) OR (season>=2024) 라는
    **이접(disjunction)** 이라, 트리가 축 정렬 분할로 표현하기 어렵다. 명시 가치가 있다.
    평가(2025)는 두 리그 모두 새 데이터 체제라고 가정해 1.
    """
    out = df.copy()
