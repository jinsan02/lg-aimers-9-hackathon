...
# v14 = v13 + refit 배수 1.5 + 실패모드 셀 다중분류 멤버.
# 가중 0.20 은 최적화가 아니라 **강건성 선택**이다 — 2024 자기적합 최적은 0.238,
# 2023R 최적은 0.543 으로 시즌마다 크게 다르다. 0.543 을 2024 에 적용하면 -1.71 이다.
# 0.20 은 2024 +2.65 / 2023 +6.96 / 최악(margin 0) -1.80 으로 어느 쪽에서도 안전하다.
_W_CELL = 0.20
_F = [42, 7, 13, 3, 4, 5, 6, 8]
_C = [42, 7, 13, 3, 4, 5]
WEIGHTS = ([(f"./model/cat_v14f_s{s}.pkl", (1 - _W_CELL) / len(_F)) for s in _F]
           + [(f"./model/cat_v14c_s{s}.pkl", _W_CELL / len(_C)) for s in _C])
...
def blend(test):
    ...
    print(f"BLEND v6 mean={raw:.4f} -> 기울기 x{SLOPE} -> {preds.mean():.4f} "
          f"-> SHIFT -{SHIFT:.4f}")
